    ...
    <div class="col-md-12 bg-white border border-2 rounded rounded-3 shadow shadow-3 border-light p-5">
        <h2><?= isset($_GET["id"]) ? "Edit Post" : "Add Post" ?></h2>
            ...
            <div class="form-floating mb-2">
                <input type="text" class="form-control border border-3 <?= isset($errors["title"]) ? "border-danger" : "" ?>" id="title" name="title" placeholder="Enter Post title" onkeyup="validateTitle(this)" value="<?= isset($_GET["id"]) ? $data["title"] : "" ?>">
                <label for="title">Title</label>
                <p class="error-title text-danger" id="error-title" style="display:<?= isset($errors["title"]) ? "block" : "none" ?>;">Title can't be empty!</p>
            </div>
            <div class="form-floating mb-2">
                <textarea class="form-control border border-3 <?= isset($errors["description"]) ? "border-danger" : "" ?> " placeholder="Enter Description" id="description" name="description" style="height: 100px" onkeyup="validateDescription(this)" ><?= isset($_GET["id"]) ? $data["description"] : "" ?></textarea>
                <label for="description">Description</label>
                <p class="error-description text-danger" id="error-description" style="display:<?= isset($errors["description"]) ? "block" : "none" ?>;">Description can't be empty! and max 500 characters allowed </p>
            </div>
            <label for="content">Content:</label>
            <div class="form-floating mb-2 border border-3 rounded <?= isset($errors["content"]) ? "border-danger" : "" ?>" id="editor-div">
                <textarea class="form-control border border-3" id="editor" name="content"  style="height: 300px" oninput="validateContent(this)"><?= isset($_GET["id"]) ? $data["content"] : "" ?></textarea>
            </div>
                <p class="error-content text-danger" id="error-content" style="display:<?= isset($errors["content"]) ? "block" : "none" ?>">Content can't be empty!</p>
            <?php if(!isset($_GET["id"])):?>
                ...
                <label >Select Category:</label>
                    ...
                    <a href="../categories/create-category.php"  style="text-decoration:none; position:relative; right:-45%;">+Add Category</a>               
                            ...
                            <?php endif;?>
            <?php endif;?>                    
                <button type="submit" class="btn btn-primary mt-3"><?= isset($_GET["id"]) ? "Edit Post" : "Add Post" ?></button>
            </form>
        </div>
  </div>
